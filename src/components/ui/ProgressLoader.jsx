...
export const ProgressLoader = ({ statusText, progress, error, onRetry, onReset, isComplete, onFinish }) => {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center p-8 max-w-md mx-auto text-center">
        ...
        <h3 className="text-xl font-bold text-slate-200 mb-2">Генерация қатесі</h3>
        <p className="text-slate-300 mb-6">{error}</p>
        <div className="flex justify-center gap-3">
          <button onClick={onReset} className="px-4 py-2 text-sm font-medium text-slate-200 bg-paper/50 hover:bg-slate-200 rounded-xl transition-colors">
            Қайта бастау
          </button>
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 max-w-md mx-auto">
      <div className="relative mb-8">
        ...
        {isComplete && (
          <motion.div
            ...
            className="absolute -bottom-2 -right-2 text-green-500 glass-card rounded-full"
          >
      ...
      <div className="w-full bg-paper/50 h-2.5 rounded-full overflow-hidden mt-4">
        <motion.div 
